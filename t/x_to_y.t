...
eval "use Number::Tolerant";
if ($@) {
	plan skip_all => "can't run these tests without Number::Tolerant" if $@;
} else {
	plan tests => 67;
}
...
my $guess =
	encapsulate(Number::Tolerant->new(4.5 => to => 5.5));

ok($guess, "created our object");

isa_ok($guess,  "Object::Capsule", " ...");
isa_ok($$guess, "Number::Tolerant", " ... content");

is("$guess", "4.5 to 5.5", " ... stringifies properly");
